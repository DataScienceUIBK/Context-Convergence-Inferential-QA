...
class By_Server:
    def __init__(self, model_url):
        with open(f'./dataset_similarities.json', mode='r', encoding='utf8') as f:
        ...
        self.answers = []
        if os.path.exists(f'./qa_results/answers_{self.model_name}.json'):
            with open(f'./qa_results/answers_{self.model_name}.json', mode='r', encoding='utf-8') as f:
    ...
    def _prompt(self, batch, model, tokenizer):
        q_ids, sim_methods, categories, subsets, orders, prompts = zip(*batch)
        texts = [
        ...
        predict_answers = list(zip(q_ids, sim_methods, categories, subsets, orders, responses))
    ...
    def _to_prompts(self):
        ...
        for rec in self.dataset:
            ...
            for sim_method in ['hint_similarity', 'cosine_similarity']:
                for category in ['low', 'high']:
                    for subset in rec['subsets'][sim_method][category]:
                        for _ord in ['asc_hints', 'desc_hints']:
                            context = rec['subsets'][sim_method][category][subset][_ord]['passage']
                            ...
                            prompts.append((q_id, sim_method, category, subset, _ord, prompt))
        prompts = sorted(prompts, key=lambda x: len(x[5][10]['content']))
        return prompts

    def qa(self):
        os.makedirs(f'./qa_results', exist_ok=True)
        prompts = self._to_prompts()
        batch_size = 512

        print(colored(f'Loading {self.model_name} to generate answers...', 'yellow'))
        print()
        ...
        while idx < len(prompts):
            ...
            if counter % self.steps == 0 or idx >= len(prompts) - 1:
                counter = 0
                with open(f'./qa_results/answers_{self.model_name}.json', mode='w',
                          encoding='utf-8') as f:
